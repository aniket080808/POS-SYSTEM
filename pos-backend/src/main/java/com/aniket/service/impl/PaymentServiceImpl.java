...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentInitiateResponse initiatePayment(PaymentInitiateRequest request) throws PaymentException {
        try {
            ...
            // Razorpay order creation (using Orders API instead of Payment Links to avoid rate limits)
            JSONObject orderResponse = razorpayService.createOrder(currentUser, payment);
            response.setRazorpayOrderId(orderResponse.getString("id"));
            response.setRazorpayKeyId(orderResponse.getString("key_id"));
            response.setAmount(orderResponse.getInt("amount") / 100.0);
            response.setCurrency(orderResponse.getString("currency"));

            // Publish payment initiated event
            publishPaymentInitiatedEvent(payment, response.getRazorpayOrderId());
