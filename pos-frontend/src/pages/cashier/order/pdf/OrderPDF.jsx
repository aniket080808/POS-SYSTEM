...
// Create PDF document
export const OrderPDF = ({ order, currency = "INR" }) => {
  const symbol = getCurrencySymbol(order?.store?.currency || currency);
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.infoItem}>
              ...
              <Text style={styles.totalAmount}>
                {symbol}{order.totalAmount?.toFixed(2) || "0.00"}
              </Text>
            </View>
          </View>
            ...
            {order.items?.map((item, index) => (
              <View key={item.id || index} style={styles.tableRow}>
                ...
                <Text style={styles.tableCellRight}>
                  {symbol}{item.product?.sellingPrice?.toFixed(2) || "0.00"}
                </Text>
                <Text style={styles.tableCellRight}>
                  {symbol}{(item.product?.sellingPrice * item.quantity)?.toFixed(2) || "0.00"}
                </Text>
              </View>
            ))}
          </View>
